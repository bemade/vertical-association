...
class TestMembership(TestMembershipCommon):
    def test_none_membership(self):
        ...
        self.assertEqual(
            ...
            "none",
            "membership: outdated non paid subscription should keep in non-member "
            "state",
        )
    ...
    def test_old_membership(self):
        ...
        self.assertEqual(
            ...
            self.membership_1,
            "membership: new subscription should create a line with the membership "
            "as product",
        )
        self.assertEqual(
            invoice.invoice_line_ids[0].price_unit,
            75.0,
            "membership: new subscription should create a line with the given price "
        ...
        payment = self.env["account.payment"].create(
            {
                "destination_account_id": invoice.line_ids.account_id.filtered(
                    lambda account: account.account_type == "asset_receivable"
                ).id,
        ...
        payment.action_post()
        inv1_receivable = invoice.line_ids.filtered(
        ...
        self.assertEqual(
            self.partner_2.membership_state,
            "free",
            "membership: free member customer should be in free state",
        )
        self.partner_2.write(
            {"free_member": False, "associate_member": self.partner_1.id}
        )
        self.assertEqual(
            self.partner_2.membership_state,
            "old",
    ...
    def test_paid_membership(self):
        ...
        self.assertEqual(
            invoice.invoice_line_ids[0].product_id,
            self.membership_1,
            "membership: new subscription should create a line with the "
            "membership as product",
        )
        self.assertEqual(
            invoice.invoice_line_ids[0].price_unit,
            75.0,
        ...
        self.assertEqual(
            self.partner_1.membership_state,
            "invoiced",
            "membership: after opening the invoice, customer should be in invoiced "
            "status",
        )
        # the invoice is paid -> customer goes to paid status
